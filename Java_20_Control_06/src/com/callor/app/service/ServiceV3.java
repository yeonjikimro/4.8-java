 ...
 * 두 method 에 선언된 intKor, intEng, intMath 는 이름은 같지만 완전히 다른 변수(저장공간, 값이 다른)이다.
 * 
 * 총점을 scorePrint()에서 직접 계산하면 되겠지만
 * scoreSum()을 사용하여 총점을 계산하고 싶다면
...
public class ServiceV3 {
	...
	public void scorePrint() {
		/*
		 * String(문자열) type 의 변수의
		 * 실체는 String 클래스 type의 인스턴스이다.
		 * String 클래스에서는 문자열을 다루는 다양한 method 들이 이미 준비되어 있다.
		 * 다양한 method 를 사용하여 여러가지 기능을 손쉽게
		 * 사용할 수 있다. 
		 */
		// 첫글자 대문자: 클래스=String, 소문자: 변수
		String dLine = "=";
