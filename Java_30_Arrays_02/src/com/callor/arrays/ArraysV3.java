...
public class ArraysV3 {
	...
	public static void main(String[] args) {
		...
		// index 라는 변수를 사용하여 첨자를 지정함으로써 반복되는 코드가 만들어지고 단순한 코드가 생성되었다.
		...
		/*
		 * 위 코드에서 만약 데이터의 개수를 200으로 변경(new int[200]) 했을 때 for() 명령문의 index < 100 을 index
		 * < 200 으로 변경해 주어야 한다 만약 for() 명령문의 코드를 변경하지 않으면 테스트 데이터의 개수를 늘린 효과가 없어진다.
		 * 
		 * 배열의 개수를 변경할 때마다 코드를 추가로 변경하는 불편을 없애기 위해 자바에서는 배열의 크기와 관련된 내부 속성을 마련해 두었다.
		 * 배열.length 속성은 배열의 개수를 담고 있는 특별한 변수이다. 배열.length 는 배열을 생성하면 배열의 개수를 자동으로 보관하도록
		 * 자동으로 만들어지는 특별한 변수이다.
		 * 
		...
		// intScore 배열의 0 번부터 intScore.length - 1 번까지의 점수를 모두 합산하여
		// intSum 에 담고, console 에 출력
		...
		// 평균 결과를 소수점까지 표현하고 싶으면 원 데이터를 먼저 실수형으로 강제 형 변환 한 후에 연산을 하고 결과를 실수형 변수에 저장하면 된다.
