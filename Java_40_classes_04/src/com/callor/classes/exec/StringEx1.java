...
public class StringEx1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(strNation);// 아래 코드와 같은 결과
		for (int i = 0; i < strNation.length(); i++) {
			System.out.print(strNation.substring(i, i + 1));
		}
		System.out.println();
		// length : 속성(변수) -10이라는 값을 가짐(private int length = 10_
		...
		// length() : 메소드 -필요할때마다 메소드 계속 호출하여 값 반환
		...
		/*
		 * str1.length() 메서드는 문자열의 길이를 계산하는 method이다 for() 반복문이 문자열의 길이만큼 반복될 때 계속해서
		 * 문자열의 길이를 계산하는 코드가 실행된다.
		 *
		 * 
		 */
		/*
		 * for() 반복문이 실행되기 전에 문자열의 개수를 계산하여 return 하는 length() 메서드를 호출하고 결과를 변수에 담자 이제는
		 * 문자열의 길이를 계산하는 코드는 한번만 실행되고 문자열의 길이만큼 for() 반복문을 수행할 수 있게 되었다. 상당히 많은 코드 실행이
		 * 없어져 효율적인 코드가 된다
		 */
