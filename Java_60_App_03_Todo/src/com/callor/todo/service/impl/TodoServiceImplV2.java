...
public class TodoServiceImplV2 extends TodoServiceImplV1 {
	 ...
	 * 
	 * todolist.txt 파일 열기 열기할 파일은 V1 의 saveFileName 에 저장되어 있다. 한라인씩 읽어서 컴마(,) 기준으로
	 * split 하고 todoVO 에 담고 todoList 에 추가 V1 의 todoList 객체가 생성되어 있다.
	 */
	...
	protected void loadTodoList() {
		...
		while (scan.hasNext()) {
			...
			/*
			 * text 파일에 데이터를 저장하면 마지막에 공백의 Line 이 추가되는 경우가 있다. 이때는 split 으로 이러한 상황을 방지하기 위하여
			 * 아래 코드를 추가한다
			 */
			...
			TodoVO tVO = TodoVO.builder().tKey(todo[T_KEY]).sdate(todo[T_SDATE]).stime(todo[T_STIME]).edate(eDate)
					.etime(eTime).tContent(todo[T_CONTENT]).build();
